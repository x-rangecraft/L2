...
from .command_watchdog import CommandWatchdog
    ...
    def _create_ros_interfaces(self) -> None:
        self._subscriptions.append(
            self.create_subscription(
                PoseStamped,
                self._params.robot_command_topic,
                self._on_robot_command,
                10,
            )
        )
        if self._params.joint_command.enabled:
            self._subscriptions.append(
                self.create_subscription(
                    JointState,
                    self._params.joint_command_topic,
        ...
        self._subscriptions.append(
            self.create_subscription(
                Empty,
                self._params.safety_stop_topic,
